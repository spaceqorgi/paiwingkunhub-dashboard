  ...
  methods: {
    async update_bankshow (bank_id, bank_fullname, bank_show) {
      ...
      if (this.onoff_status === true) {
        this.$vs.notify({
          ...
          title: bank_show === 1 ? 'เปิดแสดง Kbank หน้าเว็บสำเร็จ' : 'ปิดแสดง Kbank หน้าเว็บสำเร็จ',
        ...
        await log.agent(bank_fullname, 'Bank_Show', 0, text)
      } else {
        this.$vs.notify({
          time: 8000,
          color: 'danger',
          position: 'top-right',
          icon: 'error',
          title: bank_show === 1 ? 'เปิดแสดง Kbank หน้าเว็บไม่สำเร็จ!!' : 'ปิดแสดง Kbank หน้าเว็บไม่สำเร็จ!!',
    ...
    async update_bankrun (bank_id, bank_fullname, bank_run) {
      ...
      if (this.onoff_status.status === true) {
        this.$vs.notify({
          ...
          title: bank_run === 1 ? 'เปิดแสดง Kbank หน้าเว็บสำเร็จ' : 'ปิดแสดง Kbank หน้าเว็บสำเร็จ',
        ...
        this.$vs.notify({
          ...
          position: 'top-right',
          icon: 'error',
          title: bank_run === 1 ? 'เปิดดึงยอด Kbank หน้าเว็บไม่สำเร็จ!!' : 'ปิดดึงยอด Kbank หน้าเว็บไม่สำเร็จ!!',
          text: 'กรุณาติดต่อทีมงานโปรแกรมเมอร์'
        })
      }
    },
    ...
    async deletedata () {
      ...
      if (this.status.status === true) {
        ...
        log.agent('', 'Del_Deposit_Kbank', 0, `ลบบัญชีฝาก Kbank เลขบัญชี : ${  this.delete_bankno} ชื่อเจ้าของบัญชี : ${  this.delete_name}`)
