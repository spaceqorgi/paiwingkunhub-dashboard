          ...
          <vs-tr :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td  v-if="tr.bank_reserve == 1" class="text-danger">
             ...
             <vs-td>
               ...
               @click="$router.push('/SettingWalletDeposit/SettingWalletedit/'+bank+'/'+tr.bank_id)">แก้ไขบัญชีเงินฝาก
                </vs-button>
                 <!-- <vs-button type="border" size="small" icon-pack="feather" icon="icon-trash" color="danger" class="mr-2" @click="popup_delbank(tr.bank_id,tr.bank_number,tr.bank_fullname)">
                  ลบบัญชีฝาก</vs-button> -->
               </div>
            </vs-td>
            ...
            <vs-td :data="tr.bank_run">
              <vs-switch v-if="tr.bank_reserve == 0" v-model="tr.bank_run" @click="update_bankrun(tr.bank_id,tr.bank_fullname,tr.bank_run)">
                <span slot="on">On</span>
                <span slot="off">Off</span>
              </vs-switch>
            </vs-td>
  ...
  methods: {
    async update_bankshow (bank_id, bank_fullname, bank_show) {
      ...
      if (this.onoff_status === true) {
        this.$vs.notify({
          ...
          title: bank_show === 1 ? 'เปิดแสดง scbonly หน้าเว็บสำเร็จ' : 'ปิดแสดง scbonly หน้าเว็บสำเร็จ',
        ...
        this.$vs.notify({
          ...
          position: 'top-right',
          icon: 'error',
          title: bank_show === 1 ? 'เปิดแสดง scbonly หน้าเว็บไม่สำเร็จ!!' : 'ปิดแสดง scbonly หน้าเว็บไม่สำเร็จ!!',
    ...
    async update_bankrun (bank_id, bank_fullname, bank_run) {
      ...
      if (this.onoff_status.status === true) {
        this.$vs.notify({
          ...
          title: bank_run === 1 ? 'เปิดแสดง scbonly หน้าเว็บสำเร็จ' : 'ปิดแสดง scbonly หน้าเว็บสำเร็จ',
        ...
        this.$vs.notify({
          ...
          position: 'top-right',
          icon: 'error',
          title: bank_run === 1 ? 'เปิดดึงยอด scbonly หน้าเว็บไม่สำเร็จ!!' : 'ปิดดึงยอด scbonly หน้าเว็บไม่สำเร็จ!!',
          text: 'กรุณาติดต่อทีมงานโปรแกรมเมอร์'
        })
      }
    },
    ...
    async deletedata () {
      ...
      if (this.status.status === true) {
        ...
        log.agent('', 'Del_Deposit_ScbOnly', 0, `ลบบัญชีฝาก ScbOnly เลขบัญชี : ${  this.delete_bankno} ชื่อเจ้าของบัญชี : ${  this.delete_name}`)
