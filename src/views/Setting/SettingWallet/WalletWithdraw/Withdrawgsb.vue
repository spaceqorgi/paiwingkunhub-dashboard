<template>
  <div>
    ...
    <vx-card title="BANK GSB">
      
      <vs-table stripe :data="bankgsb">
  ...
  methods: {
    ...
    async deletedata () {
      ...
      if (this.status.status === false) {
        ...
        log.agent('', 'Del_Withdraw_Gcb', 0, `ลบบัญชีถอน Gsb เลขบัญชี : ${  this.delete_bankno} ชื่อเจ้าของบัญชี : ${  this.delete_name}`)
