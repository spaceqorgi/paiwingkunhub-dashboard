<template>
  <div>
    <vx-card title="BANK SCB">
      <vs-table stripe :data="bankscb_data">
  ...
  methods: {
    ...
    async deletedata () {
      ...
      if (this.status.status === false) {
        ...
        log.agent('', 'Del_Withdraw_Scb', 0, `ลบบัญชีถอน Scb เลขบัญชี : ${  this.delete_bankno} ชื่อเจ้าของบัญชี : ${  this.delete_name}`)
